...
export const AIPlanner = ({ onClose }) => {
    ...
    const BudgetOption = ({ value, label, icon: Icon }) => (
        ...
        >
            <Icon size={20} className={budget === value ? "text-amber-600" : "text-slate-400"}/>
    ...
    return (
        <div className="fixed inset-0 z-[80] flex items-center justify-center p-4">
            ...
            <motion.div 
                ...
                className="bg-white w-full max-w-lg rounded-3xl relative z-10 border border-slate-100 shadow-2xl overflow-hidden flex flex-col max-h-[85vh]"
            >
                {/* Header */}
                <div className="p-5 border-b border-slate-100 flex justify-between items-center bg-white sticky top-0 z-20">
                    <div className="flex items-center gap-2">
                        ...
                        <div>
                            <h3 className="text-lg font-black text-slate-900 tracking-tight">YARD AI</h3>
                        ...
                        {step === 1 ? (
                            ...
                            >
                                ...
                                <div className="space-y-2">
                                    <label className="text-sm font-bold text-slate-700 flex items-center gap-2">
                                        <Users size={16} className="text-amber-500"/> Total Guests
                                    </label>
                                    <input 
                                        type="number" 
                                        inputMode="numeric"
                                        ...
                                        className="w-full bg-slate-50 p-4 rounded-xl text-slate-900 font-bold border border-slate-200 focus:border-amber-500 focus:ring-4 focus:ring-amber-500/10 outline-none transition-all placeholder:text-slate-400 text-lg" 
                                        placeholder="e.g. 30" 
                                    />
                                </div>
                                ...
                                <div className="space-y-2">
                                    <label className="text-sm font-bold text-slate-700 flex items-center gap-2">
                                        <PartyPopper size={16} className="text-amber-500"/> Event Type
                                    </label>
                                    <div className="relative">
                                        <select 
                                            value={eventType} 
                                            onChange={e => setEventType(e.target.value)} 
                                            className="w-full bg-slate-50 p-4 rounded-xl text-slate-900 font-bold border border-slate-200 focus:border-amber-500 outline-none appearance-none cursor-pointer"
                                        >
                                ...
                                <div className="space-y-2">
                                    <label className="text-sm font-bold text-slate-700 flex items-center gap-2">
                                        <Wallet size={16} className="text-amber-500"/> Budget Level
                                    </label>
                            ...
                            >
                                ...
                                <div className="bg-amber-50 p-4 rounded-xl border border-amber-100 flex gap-3 items-start">
                                    <Sparkles className="text-amber-600 flex-shrink-0 mt-1" size={18}/>
                                    <div>
                                        <h4 className="font-bold text-amber-800 text-sm mb-1">AI Recommendation</h4>
                                        <p className="text-sm text-amber-900/80 italic leading-relaxed">"{result?.summary}"</p>
                                    </div>
                                </div>
                                    ...
                                    return (
                                      <div key={i} className="flex gap-3 bg-white p-3 rounded-xl border border-slate-100 shadow-sm hover:shadow-md transition-shadow">
                                        ...
                                        <div className="flex-grow">
                                          ...
                                          {rec.selectedOption && (
                                              <div className="text-[10px] uppercase tracking-wider text-amber-700 font-bold bg-amber-50 inline-block px-1.5 py-0.5 rounded mt-1 border border-amber-100">
                                                  {rec.selectedOption}
                                              </div>
                                          )}
                                ...
                                {/* Action Buttons */}
                                <div className="flex gap-3 pt-4 sticky bottom-0 bg-white pb-2 border-t border-slate-100 mt-2">
                                    <button 
                                        onClick={() => setStep(1)} 
                                        className="flex-1 py-3 bg-slate-100 text-slate-600 font-bold rounded-xl hover:bg-slate-200 transition-colors flex justify-center items-center gap-2"
                                    >
                                        <ChevronLeft size={18}/> Retry
                                    </button>
                                    <button 
                                        onClick={handleAccept} 
                                        className="flex-[2] py-3 bg-slate-900 text-white font-bold rounded-xl hover:bg-amber-600 transition-all flex justify-center items-center gap-2 shadow-lg active:scale-95"
                                    >
