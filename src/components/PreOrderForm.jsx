                ...
                >
                    <div className="flex justify-between items-center mb-6">
                        ...
                        <div className="flex gap-1">
                            <button onClick={() => setDisplayDate(d => new Date(d.getFullYear(), d.getMonth() - 1, 1))} className="p-2 hover:bg-old-lace rounded-lg"><ChevronLeft size={20}/></button>
                            <button onClick={() => setDisplayDate(d => new Date(d.getFullYear(), d.getMonth() + 1, 1))} className="p-2 hover:bg-old-lace rounded-lg"><RightArrow size={20}/></button>
                        </div>
                    </div>
                            ...
                            return (
                                <button 
                                    ...
                                    className={`w-full aspect-square rounded-xl text-sm font-bold transition-all ${
                                        ...
                                        isDisabled ? 'text-slate-300 cursor-not-allowed' : 'text-slate-700 hover:bg-almond-silk hover:text-flag-red'
                                    }`}
                                >
                                    {dayObj.date.getDate()}
                                </button>
                    ...
                    <button onClick={() => { onSelect(selectedDate); onClose(); }} disabled={!selectedDate} className="w-full py-3.5 mt-6 bg-slate-900 text-white font-bold rounded-xl hover:bg-flag-red transition-all disabled:opacity-50">
                        Confirm Date
                    </button>
                </motion.div>
            </motion.div>
        ...
        <AnimatePresence>
            ...
            >
                ...
                >
                    ...
                    <button onClick={handleConfirm} className="w-full py-3.5 bg-slate-900 text-white font-bold rounded-xl hover:bg-flag-red transition-all">Confirm Time</button>
                </motion.div>
            </motion.div>
        </AnimatePresence>
    );
        ...
        <div className="fixed inset-0 z-[80] flex items-end md:items-center justify-center p-0 md:p-4">
          ...
          >
            ...
            <div className="p-6 space-y-5 overflow-y-auto flex-grow">
              <div>
                  <label className="text-sm font-bold text-slate-600 mb-2 flex gap-2"><User size={16} className="text-flag-red"/> Name</label>
                  <input type="text" placeholder="Your Name" value={formData.name} onChange={e => setFormData({...formData, name: e.target.value})} className="w-full bg-old-lace rounded-xl px-4 py-3.5 border border-transparent focus:border-flag-red focus:bg-white outline-none transition-all font-bold text-slate-900 placeholder:text-slate-400 font-sans"/>
              </div>
              
              <div className="grid grid-cols-2 gap-4">
                 <div>
                    <label className="text-sm font-bold text-slate-600 mb-2 flex gap-2"><Calendar size={16} className="text-flag-red"/> Date</label>
                    <div onClick={() => setModalOpen('date')} className="bg-old-lace border border-transparent rounded-xl px-4 py-3.5 flex justify-between items-center cursor-pointer hover:bg-almond-silk">
                        <span className={`font-bold ${formData.date ? 'text-slate-900' : 'text-slate-400'}`}>{formatDate(formData.date)}</span>
                        <ChevronRight size={18} className="text-slate-400"/>
                    </div>
                 </div>
                 <div>
                    <label className="text-sm font-bold text-slate-600 mb-2 flex gap-2"><Clock size={16} className="text-flag-red"/> Time</label>
                    <div onClick={() => setModalOpen('time')} className="bg-old-lace border border-transparent rounded-xl px-4 py-3.5 flex justify-between items-center cursor-pointer hover:bg-almond-silk">
              ...
              <div>
                 <label className="text-sm font-bold text-slate-600 mb-2 flex gap-2"><MessageSquare size={16} className="text-flag-red"/> Notes</label>
                 <textarea className="w-full bg-old-lace rounded-xl px-4 py-3 border border-transparent focus:border-flag-red focus:bg-white outline-none h-24 resize-none font-medium placeholder:text-slate-400 font-sans" placeholder="Optional..." onChange={e => setFormData({...formData, notes: e.target.value})}>{formData.notes}</textarea>
              </div>
            </div>

            <div className="p-6 border-t border-old-lace bg-white md:rounded-b-3xl">
