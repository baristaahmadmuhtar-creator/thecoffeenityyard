    ...
    <div className="space-y-1.5">
        ...
        {isTextArea ? (
             <textarea 
                value={value} 
                onChange={onChange} 
                className="w-full p-3 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-amber-500 focus:border-amber-500 transition outline-none font-medium"
                rows="3"
...
export const AdminDashboard = () => {
  ...
  useEffect(() => {
    // Perhatikan: Collection name di sini adalah "menu" (sesuai kode kamu sebelumnya)
    ...
    <div className="min-h-screen bg-slate-50 p-4 md:p-8 font-sans text-slate-800">
      <div className="max-w-7xl mx-auto space-y-8">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
            ...
            <div className="flex flex-wrap gap-2">
                ...
                <button onClick={() => { resetForm(); setIsCreating(true); }} className="px-6 py-3 bg-slate-900 text-white font-bold rounded-xl hover:bg-amber-600 transition flex items-center gap-2 shadow-lg">
        ...
        {showCatManager && (
            <div className="bg-white p-6 rounded-2xl shadow-sm border border-amber-100 animate-in slide-in-from-top-4">
                ...
                <form onSubmit={handleAddCategory} className="flex gap-2">
                    <input 
                        ...
                        className="flex-grow p-3 bg-slate-50 border border-slate-200 rounded-xl outline-none font-medium focus:ring-2 focus:ring-amber-500"
                    />
                    <button type="submit" disabled={!newCategoryName.trim()} className="px-6 py-2 bg-amber-500 text-white font-bold rounded-xl hover:bg-amber-600 disabled:opacity-50 transition">Add</button>
                </form>
            </div>
        )}
        ...
        <div className="bg-white p-4 rounded-2xl shadow-sm border border-slate-100 flex flex-col md:flex-row justify-between items-center gap-4 sticky top-4 z-30">
            <div className="relative w-full md:w-96">
                ...
                <input type="text" placeholder="Search menu..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="w-full pl-10 pr-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-amber-500 transition outline-none"/>
            </div>
            <div className="flex items-center gap-3 w-full md:w-auto">
                 <select value={filterCategory} onChange={(e) => setFilterCategory(e.target.value)} className="px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl font-bold text-sm text-slate-600 outline-none">
                     <option value="All">All Categories</option>
                     {categories.map(c => <option key={c} value={c}>{c}</option>)}
                 </select>
                 <div className="flex bg-slate-100 p-1 rounded-xl shrink-0">
                     <button onClick={() => setViewMode('grid')} className={`p-2 rounded-lg transition ${viewMode === 'grid' ? 'bg-white shadow text-amber-600' : 'text-slate-400'}`}><LayoutGrid size={20}/></button>
                     <button onClick={() => setViewMode('list')} className={`p-2 rounded-lg transition ${viewMode === 'list' ? 'bg-white shadow text-amber-600' : 'text-slate-400'}`}><List size={20}/></button>
                 </div>
            </div>
        </div>
                    ...
                    <div key={item.id} className="bg-white rounded-2xl border border-slate-100 shadow-sm hover:shadow-xl transition-all duration-300 overflow-hidden group flex flex-col">
                        <div className="h-48 relative overflow-hidden bg-slate-100">
                            ...
                            <div className="absolute top-2 right-2 flex gap-1">
                                <button onClick={() => startEdit(item)} className="p-2 bg-white/90 backdrop-blur rounded-lg hover:bg-amber-500 hover:text-white transition shadow-sm"><Edit size={16}/></button>
                            ...
                            {item.badge && (
                                <div className="absolute bottom-2 left-2 bg-amber-500 text-white text-[10px] font-bold px-2 py-1 rounded shadow-sm">
                                    {item.badge}
                                </div>
                            )}
                        </div>
                        ...
                        <div className="p-5 flex-grow flex flex-col">
                            <div className="flex justify-between items-start mb-2">
                                ...
                                <div className="text-right">
                                    ...
                                    <span className="font-black text-amber-600 text-lg">{formatBND(item.price)}</span>
                                </div>
                            </div>
                        ...
                        {filteredMenu.map(item => (
                            <tr key={item.id} className="hover:bg-slate-50/80 transition-colors">
                                ...
                                <td className="p-5">
                                    ...
                                    <span className="font-bold text-amber-600">{formatBND(item.price)}</span>
                                </td>
                                <td className="p-5">
                                    <div className="flex items-center gap-3">
                                        <button onClick={() => adjustStock(item, -1)} className="w-8 h-8 flex items-center justify-center rounded-lg bg-slate-100 hover:bg-amber-100 transition"><Minus size={14}/></button>
                                        <span className="font-bold text-slate-700">{item.stock}</span>
                                        <button onClick={() => adjustStock(item, 1)} className="w-8 h-8 flex items-center justify-center rounded-lg bg-slate-100 hover:bg-green-100 transition"><Plus size={14}/></button>
                                    </div>
                                </td>
                                <td className="p-5">
      ...
      {(isCreating || editingId) && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm">
            ...
            >
                 <button type="button" onClick={resetForm} className="absolute top-6 right-6 p-2 rounded-full hover:bg-slate-100 transition"><X size={20}/></button>
                 
                 <div className="flex items-center gap-3 mb-6">
                    {editingId && <div className="px-3 py-1 bg-amber-100 text-amber-800 text-xs font-bold rounded-full">Editing Mode</div>}
                    <h2 className="text-2xl font-black text-slate-900">{isCreating ? 'Add New Product' : 'Edit Product'}</h2>
                 </div>
                 ...
                 <div className="space-y-6">
                    ...
                    <div className="grid md:grid-cols-2 gap-6">
                        ...
                        <div className="space-y-1.5">
                            ...
                            <select value={formData.category} onChange={e => setFormData({...formData, category: e.target.value})} className="w-full p-3 bg-slate-50 border border-slate-200 rounded-xl outline-none font-medium">
                    ...
                    <div className="grid grid-cols-2 gap-4 bg-amber-50 p-4 rounded-xl border border-amber-100">
                        <Input label="Original Price (Coret)" type="number" placeholder="Optional" value={formData.originalPrice} onChange={e => setFormData({...formData, originalPrice: e.target.value})} />
                        <Input label="Selling Price (Jual)" type="number" value={formData.price} onChange={e => setFormData({...formData, price: e.target.value})} required/>
                    </div>
                    ...
                    <div className="bg-slate-50 p-4 rounded-xl border border-slate-200 space-y-4">
                        ...
                        <div className="grid grid-cols-2 gap-4">
                             <Input label="Mix Limit (Bundle Only)" type="number" value={formData.mixLimit} onChange={e => setFormData({...formData, mixLimit: e.target.value})} placeholder="e.g. 3"/>
                             
                             <div className="flex flex-col gap-2 pt-1">
                                <label className="text-xs font-bold text-slate-500 uppercase tracking-wide">Mix Type</label>
                                <label className="flex items-center gap-2 cursor-pointer bg-white p-2.5 rounded-xl border border-slate-200 hover:border-amber-400 transition">
                                    <input 
                                        type="checkbox" 
                        ...
                        <div className="space-y-1.5">
                            ...
                            <div className="flex gap-4">
                                <label className="flex items-center gap-2 cursor-pointer">
                                    <input type="radio" name="status" checked={formData.isAvailable === true} onChange={() => setFormData({...formData, isAvailable: true})} className="w-5 h-5 text-amber-600"/>
                                    <span className="text-sm font-bold text-slate-700">Active (Visible)</span>
                                </label>
                                <label className="flex items-center gap-2 cursor-pointer">
                                    <input type="radio" name="status" checked={formData.isAvailable === false} onChange={() => setFormData({...formData, isAvailable: false})} className="w-5 h-5 text-amber-600"/>
                    ...
                    <button type="submit" className="w-full py-4 bg-slate-900 text-white font-bold rounded-xl hover:bg-amber-600 transition shadow-lg flex justify-center items-center gap-2 mt-4">
